...
def data_fit():
        ...
        #Categorizo el dataset rfm_scaled
        print('Categorizando el dataset rfm_scaled con K-Means')
        clus = KMeans(n_clusters=4, n_init=10, init= 'k-means++', max_iter= 300)
        clus.fit(rfm_scaled)

        #Guardo Modelo en disco
...
with DAG('KMEANS-FIT',
        ...
        t_data_fit = PythonOperator(task_id='data_fit', python_callable=data_fit, dag=dag)         
        t_end = DummyOperator(task_id='end')
        t_start >> t_load_datasets >> t_data_process >> t_data_scaler >> t_data_fit  >> t_end    
